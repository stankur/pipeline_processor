...
def finalize_user_profile(username: str) -> None:
    """Persist user theme and highlighted repos onto user's subject.

    This task expects infer_user_theme to have succeeded; it tries best-effort write.
    """
    print(f"[task] finalize_user_profile start username={username}")
    conn = get_conn()
    set_work_status(conn, "finalize_user_profile", "user", username, "running")
    ...
    # Find infer_user_theme result from work_items
    row = conn.execute(
        "SELECT output_json FROM work_items WHERE kind='infer_user_theme' AND subject_type='user' AND subject_id=? AND status='succeeded'",
        (username,),
    ).fetchone()
    theme = None
    highlights: List[str] = []
    ...
    if theme is not None:
        ubase["theme"] = theme
    # Always set highlighted list (can be empty)
    ubase["highlighted_repos"] = highlights
    upsert_subject(conn, "user", username, json.dumps(ubase))

    set_work_status(conn, "finalize_user_profile", "user", username, "succeeded", json.dumps({"highlighted": len(highlights)}))
    conn.commit()
    print(f"[task] finalize_user_profile done username={username} highlights={len(highlights)}")
